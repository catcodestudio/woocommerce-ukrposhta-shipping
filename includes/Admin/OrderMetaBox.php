...
class OrderMetaBox {
	...
	public function add( $screen_id, $post_or_order ): void {
		...
		add_meta_box(
			'upwc-office',
			__( 'Укрпошта', 'ukrposhta-shipping-for-woocommerce' ),
			array( $this, 'render' ),
			$screen_id,
			'side',
			'default'
	...
	public function render( $post_or_order ): void {
		...
		if ( '' === $office && '' === $postindex ) {
			echo '<p>' . esc_html__( 'Для цього замовлення Укрпошту не обирали.', 'ukrposhta-shipping-for-woocommerce' ) . '</p>';
			return;
		}
		?>
		<p>
			<strong><?php esc_html_e( 'Місто:', 'ukrposhta-shipping-for-woocommerce' ); ?></strong>
			<?php echo esc_html( '' !== $city ? $city : '—' ); ?><br>
			<strong><?php esc_html_e( 'Відділення:', 'ukrposhta-shipping-for-woocommerce' ); ?></strong>
			<?php echo esc_html( '' !== $office ? $office : '—' ); ?><br>
			<strong><?php esc_html_e( 'Індекс:', 'ukrposhta-shipping-for-woocommerce' ); ?></strong>
