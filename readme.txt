...
= 1.2.7 =
* Fixed: the Pro version activated next to the free copy from WordPress.org no longer loads the free copy's code with PHP warnings on every request. Pro pauses with a notice to deactivate the free copy, and WooCommerce no longer lists it as incompatible with HPOS.

= 1.2.6 =
* Fixed: uninstalling the plugin now also removes its settings (woocommerce_ukrposhta_settings, including the API tokens) and the city search cache.

= 1.2.5 =
* Fixed: a guest who creates an account at checkout (or on the order confirmation page) no longer gets the post office as the new account's address.
...
* Fixed: on the block checkout, when the only available rate was another method, the picker still opened and wrote the office into the address fields; the chosen rate is now read from the cart. The international rate no longer opens the post-office picker either.
* Switching from Ukrposhta to another method puts the shopper's own address back into the fields instead of the office.
